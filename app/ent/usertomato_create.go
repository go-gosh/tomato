...
func (utc *UserTomatoCreate) Save(ctx context.Context) (*UserTomato, error) {
	...
	if len(utc.hooks) == 0 {
		...
		nv, ok := v.(*UserTomato)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from UserTomatoMutation", v)
		}
		node = nv
	}
	return node, err
...
func (utc *UserTomatoCreate) check() error {
	if _, ok := utc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "UserTomato.created_at"`)}
	}
	if _, ok := utc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "UserTomato.updated_at"`)}
	}
	if _, ok := utc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`ent: missing required field "UserTomato.user_id"`)}
	}
	if _, ok := utc.mutation.StartTime(); !ok {
		return &ValidationError{Name: "start_time", err: errors.New(`ent: missing required field "UserTomato.start_time"`)}
	}
	if _, ok := utc.mutation.Color(); !ok {
		return &ValidationError{Name: "color", err: errors.New(`ent: missing required field "UserTomato.color"`)}
	}
	if v, ok := utc.mutation.Color(); ok {
		if err := usertomato.ColorValidator(v); err != nil {
			return &ValidationError{Name: "color", err: fmt.Errorf(`ent: validator failed for field "UserTomato.color": %w`, err)}
		}
	}
	if _, ok := utc.mutation.RemainTime(); !ok {
		return &ValidationError{Name: "remain_time", err: errors.New(`ent: missing required field "UserTomato.remain_time"`)}
	}
	if _, ok := utc.mutation.UsersID(); !ok {
		return &ValidationError{Name: "users", err: errors.New(`ent: missing required edge "UserTomato.users"`)}
	}
	return nil
}
