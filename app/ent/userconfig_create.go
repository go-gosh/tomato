...
func (ucc *UserConfigCreate) check() error {
	...
	if _, ok := ucc.mutation.Rank(); !ok {
		return &ValidationError{Name: "rank", err: errors.New(`ent: missing required field "UserConfig.rank"`)}
	}
	if _, ok := ucc.mutation.Working(); !ok {
		return &ValidationError{Name: "working", err: errors.New(`ent: missing required field "UserConfig.working"`)}
	}
	if _, ok := ucc.mutation.Break(); !ok {
		return &ValidationError{Name: "break", err: errors.New(`ent: missing required field "UserConfig.break"`)}
	}
	if _, ok := ucc.mutation.UsersID(); !ok {
		return &ValidationError{Name: "users", err: errors.New(`ent: missing required edge "UserConfig.users"`)}
	}
	return nil
}
