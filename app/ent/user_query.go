...
type UserQuery struct {
	...
	withUserTomatoes *UserTomatoQuery
...
func (uq *UserQuery) QueryUserConfigs() *UserConfigQuery {
	query := &UserConfigQuery{config: uq.config}
...
func (uq *UserQuery) QueryUserTomatoes() *UserTomatoQuery {
	query := &UserTomatoQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserConfigs:  uq.withUserConfigs.Clone(),
		withUserTomatoes: uq.withUserTomatoes.Clone(),
...
func (uq *UserQuery) WithUserConfigs(opts ...func(*UserConfigQuery)) *UserQuery {
	...
	uq.withUserConfigs = query
	return uq
}

// WithUserTomatoes tells the query-builder to eager-load the nodes that are connected to
// the "user_tomatoes" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserTomatoes(opts ...func(*UserTomatoQuery)) *UserQuery {
	query := &UserTomatoQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserTomatoes = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [2]bool{
			uq.withUserConfigs != nil,
			uq.withUserTomatoes != nil,
		}
	)
	...
	if query := uq.withUserConfigs; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserConfigs = []*UserConfig{}
		}
		query.withFKs = true
		query.Where(predicate.UserConfig(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserConfigsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_user_configs
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_configs" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_configs" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserConfigs = append(node.Edges.UserConfigs, n)
		}
	}

	if query := uq.withUserTomatoes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
